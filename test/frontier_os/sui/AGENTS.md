...
| `transaction_builder_ptb_test.exs` | `PTBTest` | 20 | R1-R20: All PTB BCS encoders (argument, call_arg, object_ref, gas_data, expiration, type_tag, struct_tag, move_call, command, programmable_transaction, transaction_data) |
| `transaction_builder_test.exs` | `TransactionBuilderTest` | 14 | R21-R34: build!/1, build/1, digest/1, execute/3, reference vector |
...
- transaction_builder_test: `@reference_tx_hex` inline hex fixture for R34 reference vector
- transaction_builder_ptb_test: Independent expected_* helpers mirror each encoder for byte-level verification
- No mocks except ClientMock (used by SVC_SuiClient and SVC_TransactionBuilder execute/3)
